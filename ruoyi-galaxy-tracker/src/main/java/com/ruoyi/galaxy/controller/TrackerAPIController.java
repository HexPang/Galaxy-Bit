...
public class TrackerAPIController extends BaseController {
    ...
    public boolean checkCheating (AnnounceVO announceVO, SysUser user) {
        if (announceVO.getDownloaded() < 0 || announceVO.getUploaded() < 0) {
            log.error("[CHEAT] 用户令牌: " + announceVO.getToken() + " 节点编号: " + announceVO.getPeer_id() + " 下载或上传数据不正常: " + announceVO.getDownloaded() + ":" + announceVO.getUploaded());
            banded(getIp(), 1800);
            return true;
        }
        ...
        if (peers.size() > 2) {
            // 此用户有多个IP!有问题
            log.error("[CHEAT] 用户令牌: " + announceVO.getToken() + " 节点编号: " + announceVO.getPeer_id() + " IP地址数量: " + peers.size());
            banded(getIp(), 1800);
            return true;
        }
